                  ...
                  uintptr_t *result)
{
        ...
        switch (which) {
                ...
                case MMCBR_IVAR_MAX_DATA:
                        *(int *)result = (MT_MSC_DMA_MAX_SIZE *
                                          MT_MMC_MAX_BD) / MMC_SECTOR_SIZE;
                        break;
                default:
                        return (EINVAL);
        }
...
mt_mmc_setup_dma(struct mt_mmc_softc *sc)
{
        ...
        error = bus_dma_tag_create(bus_get_dma_tag(sc->dev), 4, 0,
            BUS_SPACE_MAXADDR_32BIT, BUS_SPACE_MAXADDR, NULL, NULL,
            (bus_size_t)MT_MSC_DMA_MAX_SIZE * MT_MMC_MAX_BD, MT_MMC_MAX_BD,
            MT_MSC_DMA_MAX_SIZE, 0, NULL, NULL, &sc->sc_dma_buf_tag);
        if (error != 0) {
